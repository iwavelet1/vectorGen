(function () {
  ...
  const tabVtrades = document.getElementById("tab-vtrades");
  ...
  const leftVtrades = document.getElementById("left-vtrades");
  const mainInfo = document.getElementById("main-info");
  const mainPlot = document.getElementById("main-plot");
  const mainRaw = document.getElementById("main-raw");
  const mainVtrades = document.getElementById("main-vtrades");
  const vtPlaceholder = document.getElementById("vt-placeholder");
  const vtTableWrap = document.getElementById("vt-table-wrap");
  const vtSummary = document.getElementById("vt-summary");
  const vtBody = document.getElementById("vt-body");
  const vtSelAsset = document.getElementById("vt-sel-asset");
  const vtSelDate = document.getElementById("vt-sel-date");
  const vtSelTf = document.getElementById("vt-sel-tf");
  const btnVtShow = document.getElementById("btn-vt-show");
  ...
  btnVtShow.addEventListener("click", onVtShowClick);
  ...
  tabVtrades.addEventListener("click", function () { switchTab("vtrades"); });
  ...
  function loadClassifiedManifest() {
    function applyMeta(data) {
      if (data) {
        ...
        fillSelect(vtSelAsset, data.assets || []);
        fillSelect(vtSelDate, data.dates || []);
        fillSelect(vtSelTf, data.tf || []);
      }
    }
  ...
  var TIER_ORDER = ["elite", "high_quality", "tradable", "difficult", "low_edge", "non_tradable"];

  function tierColor(tier) {
    if (!tier) return "#999";
    var t = String(tier).toLowerCase().trim();
    return TIER_COLORS[t] || "#999";
  }

  function drawLineSeries(ctx, series, xFromMin, yFromPrice, color, dash) {
  ...
  function drawVectorsPlot(data, tf) {
    ...
    const padding = { top: 24, right: 140, bottom: 52, left: 54 };
    ...
    var allSeries = [].concat(data.htf_vwap_series || []);
    let minP = Infinity;
    let maxP = -Infinity;
    ...
    drawLineSeries(ctx, data.htf_vwap_series, xFromMin, yFromPrice, "#0d47a1", [2, 3]);
    ...
    segments.forEach(function (s) {
      ...
      ctx.strokeStyle = tierColor(s.tier);
      ctx.lineWidth = 2.5;
      ctx.setLineDash([]);
      ctx.beginPath();
      ctx.moveTo(x1, y1);
    ...
    segments.forEach(function (s) {
      ...
      plotCircles.push({ x: x1, y: y1, segment_id: s.segment_id, type: "start", time_hm: s.start_hm, price: s.close_first, peak: isPeak, tier: s.tier });
      plotCircles.push({ x: x2, y: y2, segment_id: s.segment_id, type: "end", time_hm: s.end_hm, price: s.close_last, peak: !isPeak, tier: s.tier });
    });
    ...
    segments.forEach(function (s) {
      ...
      ctx.strokeStyle = "#fff";
      ctx.lineWidth = 1.5;
      [ [x1, y1], [x2, y2] ].forEach(function (xy) {
        ctx.fillStyle = tc;
        ctx.beginPath();
    ...
    var plotTrades = data.trades || [];
    plotTrades.forEach(function (t) {
      if (t.entry_min != null && t.entryPx != null && Number.isFinite(t.entryPx)) {
        var x = xFromMin(t.entry_min);
        var y = yFromPrice(t.entryPx);
        ctx.fillStyle = t.dir === "S" ? "#C62828" : "#2E7D32";
        ctx.beginPath();
        ctx.moveTo(x, y - 8); ctx.lineTo(x - 5, y + 4); ctx.lineTo(x + 5, y + 4);
        ctx.closePath(); ctx.fill();
      }
      if (t.exit_min != null && t.exitPx != null && Number.isFinite(t.exitPx)) {
        var x = xFromMin(t.exit_min);
        var y = yFromPrice(t.exitPx);
        ctx.fillStyle = "#555";
        ctx.beginPath();
        ctx.moveTo(x, y + 8); ctx.lineTo(x - 5, y - 4); ctx.lineTo(x + 5, y - 4);
        ctx.closePath(); ctx.fill();
    ...
    TIER_ORDER.forEach(function (t) {
      ...
      ctx.fill();
      ctx.fillStyle = "#333";
      ctx.fillText(t, legendX + 16, legendY);
      legendY += legendLineH;
    });
    legendY += 6;

    ctx.save();
    ctx.strokeStyle = "#0d47a1"; ctx.lineWidth = 1.5; ctx.setLineDash([2, 3]);
    ctx.beginPath(); ctx.moveTo(legendX, legendY); ctx.lineTo(legendX + 20, legendY); ctx.stroke();
    ctx.restore();
    ctx.fillStyle = "#333"; ctx.fillText("htfVwap", legendX + 26, legendY);
    legendY += legendLineH;

    ctx.fillStyle = "#2E7D32";
    ctx.beginPath(); ctx.moveTo(legendX + 6, legendY - 5); ctx.lineTo(legendX + 1, legendY + 3); ctx.lineTo(legendX + 11, legendY + 3); ctx.closePath(); ctx.fill();
    ctx.fillStyle = "#333"; ctx.fillText("Buy", legendX + 26, legendY);
    legendY += legendLineH;

    ctx.fillStyle = "#555";
    ctx.beginPath(); ctx.moveTo(legendX + 6, legendY + 5); ctx.lineTo(legendX + 1, legendY - 3); ctx.lineTo(legendX + 11, legendY - 3); ctx.closePath(); ctx.fill();
    ctx.fillStyle = "#333"; ctx.fillText("Exit", legendX + 26, legendY);
  ...
  plotCanvas.addEventListener("click", function (e) {
    ...
    if (best) {
      ...
      var tierLabel = best.tier ? " [" + best.tier + "]" : "";
      plotTooltip.textContent = best.segment_id + " " + best.type + " " + best.time_hm + " " + best.price.toFixed(2) + " (" + kind + ")" + tierLabel;
    } else {
  ...
  function onRawAlertsClick() {
    ...
    if (attrs.length === 0) attrs = source === "classified" ? ["start_time", "closing_bar_index", "rev_avwap_side_frac", "rev_avwap_cross_count", "delta_pct", "tier"] : ["time", "revDir", "close", "bar_index", "REV_avwap"];
    ...
    fetch(url)
      ...
      .then(function (data) {
        ...
        rows.forEach(function (r) { Object.keys(r).forEach(function (k) { dataKeys.add(k); }); });
        function attrInData(attr) {
          if (dataKeys.has(attr)) return true;
          if (attr === "time" && (dataKeys.has("Time") || dataKeys.has("start_time"))) return true;
          return false;
        }
        var attrsToShow = attrs.filter(attrInData);
  ...
  function onVtShowClick() {
    var asset = vtSelAsset.value;
    var date = vtSelDate.value;
    var tf = vtSelTf.value;
    vtPlaceholder.hidden = false;
    vtTableWrap.hidden = true;
    if (!asset || !date || !tf) {
      vtPlaceholder.textContent = "Select asset, date, and TF first.";
      return;
    }
    vtPlaceholder.textContent = "Loading...";
    fetch("/api/virtual-trades?asset=" + encodeURIComponent(asset) + "&date=" + encodeURIComponent(date) + "&tf=" + encodeURIComponent(tf))
      .then(function (r) { return r.json(); })
      .then(function (data) {
        var trades = data.trades || [];
        if (trades.length === 0) {
          vtPlaceholder.textContent = "No virtual trades for " + asset + " / " + date + " / " + tf + ".";
          return;
        }
        vtPlaceholder.hidden = true;
        vtTableWrap.hidden = false;
        var totalPnl = 0;
        vtBody.innerHTML = "";
        trades.forEach(function (t) {
          ...
          tr.innerHTML =
            ...
            "<td>" + (t.duration_minutes != null ? Number(t.duration_minutes).toFixed(0) : "—") + "</td>";
          vtBody.appendChild(tr);
        });
        vtSummary.textContent = trades.length + " trades, total PnL: $" + totalPnl.toFixed(2);
      })
      .catch(function () {
        vtPlaceholder.textContent = "Failed to load virtual trades.";
      });
  }
